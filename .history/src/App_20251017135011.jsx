...
const generateFlows = (map, OD, celdas, lat_min, lon_min, paso_m) => {
  if (!map) return;
  ...
  celdasArray.forEach(({ i, j, idx }) => {
    ...
    const tooltip = `<b>Celda ${idx}</b><br>
      Viajes enviados: ${intensidad_origen}<br>
      Viajes recibidos: ${intensidad_destino}<br>
      <b>Detalle:</b><br>${detalle_origenes}`;
  ...
  // --- Dibujo de las flechas animadas ---
  for (let i = 0; i < OD.length; i++) {
    for (let j = 0; j < OD[i].length; j++) {
      if (OD[i][j] > 0) {
        ...
        const line = L.polyline([origin, dest], {
          ...
          weight,
          opacity: 0.5
        }).bindPopup(`Viajes desde ${i} a ${j}: ${OD[i][j]}`).addTo(map);
        ...
        if (L.Symbol && L.Symbol.arrowHead) {
          ...
          console.warn('⚠️ leaflet-polylinedecorator no cargado correctamente.');
        }
      }
    }
  }
  ...
  const generateMap = async () => {
    ...
    try {
      ...
      if (selectedGraphType === 'heatmap') {
        ...
        generateFlows(mapRef.current, OD, celdas, lat_min, lon_min, selectedPaso);
