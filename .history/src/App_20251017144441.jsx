...
const generateODMatrix = (OD) => {
  ...
  const maxCellSize = Math.min(
    Math.floor((window.innerWidth - 200) / cols),
    Math.floor((window.innerHeight - 300) / rows)
  );
  const cellSize = Math.max(5, Math.min(maxCellSize, 30));
    ...
    <head>
      <style>
        ...
        .legend-gradient {
          ...
          border-radius: 3px;
          background: linear-gradient(to top, rgb(12, 44, 132), rgb(29, 145, 192), rgb(65, 182, 196), rgb(127, 205, 187), rgb(199, 233, 180), rgb(237, 248, 177), rgb(255, 255, 217));
        }
        .legend-labels {
          font-size: 11px;
          color: #666;
