...
const graphTypes = [
  ...
  { id: 'matrixod', name: 'Matriz OD', icon: BarChart3 }  // NUEVA LÍNEA
];
  ...
  const generateMatrixOD = (OD, celdas, paso_m) => {
  const size = OD.length;
  const cellSize = Math.max(20, Math.floor(600 / size));
  
  let htmlContent = `
    <table style="border-collapse: collapse; margin: 20px;">
      <tr style="background-color: #f0f0f0;">
        <th style="border: 1px solid #ddd; padding: 8px; width: 60px;">O/D</th>
  `;
  
  for (let j = 0; j < size; j++) {
    htmlContent += `<th style="border: 1px solid #ddd; padding: 8px; text-align: center; width: ${cellSize}px;">${j}</th>`;
  }
  htmlContent += '</tr>';
  ...
  for (let i = 0; i < size; i++) {
    htmlContent += `<tr><td style="border: 1px solid #ddd; padding: 8px; font-weight: bold; background-color: #f0f0f0;">${i}</td>`;
    for (let j = 0; j < size; j++) {
      const value = OD[i][j] || 0;
      const normalized = maxValue > 0 ? value / maxValue : 0;
      const intensity = Math.floor(255 * (1 - normalized));
      const color = `rgb(135, 206, 250)`;
      const bgColor = `rgba(135, 206, 250, ${normalized * 0.8})`;
      
      htmlContent += `<td style="border: 1px solid #ddd; padding: 8px; text-align: center; background-color: ${bgColor}; font-size: 11px;">${value}</td>`;
    }
    htmlContent += '</tr>';
  }
  htmlContent += '</table>';
  ...
  const generateMap = async () => {
    ...
    try {
      ...
      } else if (selectedGraphType === 'matrixod') {
        mapContent = generateMatrixOD(OD, celdas, selectedPaso);
      }
      ...
      const html = `
        ...
        <body>
          ${selectedGraphType === 'matrixod' ? '<div id="content" style="height: 100vh; width: 100%; overflow: auto; padding: 20px; background-color: #f5f5f5;"></div>' : '<div id="map"></div>'}
          <div class="info-box">
