...
const generateODMatrix = (OD) => {
    ...
    <head>
      <style>
        ...
        .legend-gradient {
          ...
          border-radius: 3px;
          background: linear-gradient(to top, rgb(12, 44, 132), rgb(65, 182, 196), rgb(127, 205, 187), rgb(199, 233, 180), rgb(255, 255, 217));
        }
        .legend-labels {
          font-size: 11px;
          color: #666;
  ...
  for (let i = 0; i < rows; i++) {
    for (let j = 0; j < cols; j++) {
      ...
      html += `<rect x="${x}" y="${y}" width="${cellSize}" height="${cellSize}" fill="${color}" stroke="#eee" stroke-width="0.5"/>`;
    }
  }
  
  // Labels X (Destino)
  html += `<text x="${marginLeft + cols * cellSize / 2}" y="${marginTop + rows * cellSize + 40}" text-anchor="middle" font-size="12" fill="#333" font-weight="bold">Destino</text>`;
  for (let j = 0; j < cols; j += Math.ceil(cols / 15)) {
    const x = marginLeft + j * cellSize + cellSize / 2;
    const y = marginTop + rows * cellSize + 25;
    html += `<text x="${x}" y="${y}" text-anchor="middle" font-size="10" fill="#666">${j}</text>`;
  }
  ...
  for (let i = 0; i < rows; i += Math.ceil(rows / 15)) {
    const x = marginLeft - 10;
    const y = marginTop + i * cellSize + cellSize / 2 + 4;
    html += `<text x="${x}" y="${y}" text-anchor="end" font-size="10" fill="#666">${i}</text>`;
  }
