...
for (let i = 0; i < OD.length; i++) {
  for (let j = 0; j < OD[i].length; j++) {
    if (OD[i][j] > 0) {
      ...
      const weight = 2 + (OD[i][j] / max_viajes * 6);
      const normalizado = OD[i][j] / max_viajes;
      const r = Math.floor(255 * (1 - normalizado));
      const b = Math.floor(255 * normalizado);
      const color = `rgb(${r},0,${b})`;
      
      // Color más brillante para el brillo
      const brightColor = `rgb(${Math.min(r + 100, 255)},${Math.min(b * 0.5, 255)},${Math.min(b + 100, 255)})`;
      ...
      content += `
        ...
        // Línea principal con animación fluida
        var line${i}_${j} = L.polyline([${JSON.stringify(origin)}, ${JSON.stringify(dest)}], {
          ...
          lineCap: 'round',
          lineJoin: 'round',
          className: 'animated-flow-line-main',
          dashArray: '8, 12'
        }).bindPopup('<div style="font-weight:bold; color:#333;">Flujo OD<br>Celda ${i} → Celda ${j}<br>Viajes: ${OD[i][j]}</div>').addTo(map);

        // Línea de brillo/efecto (opcional, para más elegancia)
        L.polyline([${JSON.stringify(origin)}, ${JSON.stringify(dest)}], {
          color: '${brightColor}',
          weight: ${weight * 0.4},
          opacity: 0.4,
          lineCap: 'round',
          lineJoin: 'round',
          className: 'animated-flow-line-glow',
          dashArray: '8, 12'
        }).addTo(map);
      `;
    }
...
.animated-flow-line {
  ...
  .animated-flow-line-main {
    animation: flowAnimation 3s linear infinite;
    filter: drop-shadow(0 1px 2px rgba(0,0,0,0.2));
  }

  .animated-flow-line-glow {
