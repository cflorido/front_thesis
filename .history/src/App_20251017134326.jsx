...
const generateFlows = (OD, celdas, lat_min, lon_min, paso_m) => {
    ...
    // Dibujar líneas animadas con flechas que se mueven
    for (let i = 0; i < OD.length; i++) {
      for (let j = 0; j < OD[i].length; j++) {
        if (OD[i][j] > 0) {
          ...
          // Línea base
          content += `
            var line_${i}_${j} = L.polyline([${JSON.stringify(origin)}, ${JSON.stringify(dest)}], {
              color: '${color}',
              weight: ${weight},
              opacity: 0.5
            }).bindPopup('Viajes desde celda ${i} a celda ${j}: ${OD[i][j]}').addTo(map);
            
            // Agregar flechas animadas con polylineDecorator
            var decorator_${i}_${j} = L.polylineDecorator(line_${i}_${j}, {
              patterns: [
                {
                  offset: 0,
                  repeat: 50,
            ...
            setInterval(function() {
              offset_${i}_${j} = (offset_${i}_${j} + 2) % 50;
              decorator_${i}_${j}.setPatterns([
                {
                  offset: offset_${i}_${j},
                  repeat: 50,
                  symbol: L.Symbol.arrowHead({
                    pixelSize: ${Math.max(8, weight)},
                    polygon: false,
  ...
  <body>
    ...
    <script>
      const map = L.map('map').setView([4.65, -74.1], 13);
      L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png').addTo(map);
      ${generateFlows(OD, celdas, lat_min, lon_min, paso_m)}
    </script>
  </body>
  </html>
`;
