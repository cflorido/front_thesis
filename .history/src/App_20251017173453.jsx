...
const DifferentialPrivacyVisualizer = () => {
  ...
  const [ODOriginal, setODOriginal] = useState(null);
  const [ODPerturbada, setODPerturbada] = useState(null);
  ...
  const graphTypes = [
    ...
    { id: 'flows', name: 'Flujos Oriden-Destino', icon: ArrowRightLeft }, 
        { id: 'absdiff', name: 'Mapa de Calor |Original - Perturbada|', icon: BarChart3 }
  ];
...
`;
  ...
  const generateHeatmap = (OD, celdas, lat_min, lon_min, paso_m) => {
    ...
    let rectangles = '';
celdasArray.forEach(({ i, j, idx }, index) => {
    ...
    const intensidad = Math.abs((OD_original[i]?.[j] || 0) - (OD_perturbada[i]?.[j] || 0));
    const normalizado = max_viajes > 0 ? intensidad / max_viajes : 0;
    const color = getPlasmaColor(normalizado);

    rectangles += `L.rectangle([[${lat_c - radio_lat}, ${lon_c - radio_lon}], [${lat_c + radio_lat}, ${lon_c + radio_lon}]], {color: '${color}', fillColor: '${color}', fillOpacity: 0.7, weight: 1}).bindPopup('Zona: ${idx}<br>Diferencia absoluta: ${intensidad}').addTo(map);`;
});
  ...
  const generateAbsDiffHeatmap = (OD_original, OD_perturbada, celdas, lat_min, lon_min, paso_m) => {
    const viajes_por_celda = Object.entries(celdas).map(([key, idx]) => {
        const [i, j] = key.split(',').map(Number);
        const diff = Math.abs((OD_original[i]?.[j] || 0) - (OD_perturbada[i]?.[j] || 0));
        return diff;
    });
...
if (selectedGraphType === 'destinations') {
  ...
  maxViajesForLegend = Math.max(...viajesPorCelda);
}  else if (selectedGraphType === 'absdiff' && ODOriginal && ODPerturbada) {
  const rectangles = generateAbsDiffHeatmap(ODOriginal, ODPerturbada, celdas, lat_min, lon_min, selectedPaso);
  setMapHTML(rectangles);
    ...
    filter: drop-shadow(0 0 1px rgba(255,255,255,0.5));
      ...
      ${mapContent}
          ...
          <div class="legend">
            ...
            ` : `
              <h4>Viajes (0 a ${maxViajesForLegend})</h4>
...
useEffect(() => {
  const folder = 'public'; // o la ruta correcta
  const paramStr = selectedEpsilon; // o el parámetro que uses
  const odTypeOriginal = 'original';
  const odTypePerturbada = selectedAlgorithm;

  // Cargar original
  loadCSVData(folder, paramStr, odTypeOriginal).then(({OD}) => {
    setODOriginal(OD);
  });

  // Cargar perturbada
  loadCSVData(folder, paramStr, odTypePerturbada).then(({OD}) => {
    setODPerturbada(OD);
  });
}, [selectedAlgorithm, selectedEpsilon]);
