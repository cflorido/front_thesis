...
const generateDifference = async (celdas, lat_min, lon_min, paso_m, folder, paramStr) => {
  ...
  // Calcular diferencia absoluta por celda (diagonal de la matriz)
  ...
  celdasArray.forEach(({ i, j, idx }) => {
    ...
    rectangles += `L.rectangle([[${lat_c - radio_lat}, ${lon_c - radio_lon}], [${lat_c + radio_lat}, ${lon_c + radio_lon}]], {color: '${color}', fillColor: '${color}', fillOpacity: 0.7, weight: 1}).bindPopup('Zona: ${idx}<br>Diferencia: ${diferencia.toFixed(2)} viajes').addTo(map);`;
  });
